...
export const pcForecastElements = [
  ...
  "Salary(permanent)",
  "Bonus(permanent)",
  "Payroll Tax(permananet)",
  "Leave Provision(permanent)",
  "Leave(permanent)",
...
const calculateProjects = ({
  ...
  rosterEntriesByProject.forEach(entry => {
    ...
    // all project costs
    const costCellKey = `Roster Costs-${monthLabel}`;
  ...
  projectForecastEntries.forEach(entry => {
    ...
    switch (entry.forecastType) {
      ...
      case "3": {
        ...
        // will show in main report
        const cellKey = `Fixed Price Costs-${monthLabel}`;
        const amount = +entry.amount;
...
const calculatePeopleRecovery = ({ cells, rosterEntriesByConsultant }) => {
  rosterEntriesByConsultant.forEach(entry => {
    ...
    if (
      ...
      const monthLabel = moment(entry.date).format("MMM YYYY");
      const cellKey = `Leave(permanent)-${monthLabel}`;

      if (!cells[cellKey][entry.consultant.id])
        cells[cellKey][entry.consultant.id] = 0;
...
const calculatePeopleCost = ({
  ...
  for (const month of months) {
    for (const consultant of permConsultants) {
      ...
      let cellKey = `Salary(permanent)-${month.label}`;
      if (!cells[cellKey][consultant.id]) cells[cellKey][consultant.id] = 0;
      cells[cellKey][consultant.id] += monthlySalary;
      cells[cellKey].value += monthlySalary;

      cellKey = `Bonus(permanent)-${month.label}`;
      if (!cells[cellKey][consultant.id]) cells[cellKey][consultant.id] = 0;
      cells[cellKey][consultant.id] += monthlyBonus;
      cells[cellKey].value += monthlyBonus;

      cellKey = `Payroll Tax(permananet)-${month.label}`;
      if (!cells[cellKey][consultant.id]) cells[cellKey][consultant.id] = 0;
      cells[cellKey][consultant.id] += monthlyPtax;
      cells[cellKey].value += monthlyPtax;

      cellKey = `Leave Provision(permanent)-${month.label}`;
      if (!cells[cellKey][consultant.id]) cells[cellKey][consultant.id] = 0;
      cells[cellKey][consultant.id] += monthlyLvProv;
      cells[cellKey].value += monthlyLvProv;

      // Total cost for the consultant (permanent) of the month
      const monthlyLeave =
        cells[`Leave(permanent)-${month.label}`][consultant.id] || 0;
      const monthlyCost =
        monthlySalary +
        monthlyBonus +
        monthlyLvProv +
        monthlyLeave +
        monthlyPtax;
      cellKey = `Consultant Cost(permanent)-${month.label}`;
      if (!cells[cellKey][consultant.id]) cells[cellKey][consultant.id] = 0;
      cells[cellKey][consultant.id] += monthlyCost;
      cells[cellKey].value += monthlyCost;
    }
  }
